...
full_frame_xml = "6060300f.xml"  # Full frame configuration
reduced_frame_xml = "6060300.xml"  # Reduced frame configuration
crop_size = 100  # Crop area size


def initialize_camera(config_file):
    print(f"Initializing PI 640i camera with {config_file}...")
    result = optris.usb_init(config_file)
    if result != 0:
        print(f"Failed to initialize PI 640i camera with {config_file}.")
        return False
    print(f"PI 640i camera initialized successfully with {config_file}.")
    return True
...
def process_pi_640i():
    global running, recording, frame_buffer_640i, times_computer_640i, click_x, click_y, toggled
    ...
    while running:
        ...
        if recording:  # If recording is active, save frame and time
            frame_buffer_640i.append(frame)
            times_computer_640i.append(timestamp)
            status_label.config(text=f"Saving frame at {timestamp:.2f}s")
            label_img_640i.update_idletasks()  # Update the GUI
...
def save_recording():
    ...
    if file_path:
        with open(file_path, 'wb') as f:
            np.save(f, {'frame_buffer': frame_buffer_640i, 'times_computer': times_computer_640i})
...
def create_gui():
    global label_img_640i, status_label
